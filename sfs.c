...
/****************************************************************************/
/* creates a new file called <fname> in the current directory if the
...
if (empty_dentry==-1) { empty_dentry=j; empty_dblock=i; } // AAHA! lets keep a note of it, just in case we have to create the new file
continue;
}
...
//inserting file data in first block
int flag1=0,flag2=0;
int fblock_1;
if ((fblock_1 = getBlock())==-1) {  // first get a new block using the block bitmap
printf("Error: Disk is full.\n");
return;
}else{
	
	writeSFS(fblock_1,NULL); // write all zeros to the block (there may be junk from the past!)
	itos(_inode_table[empty_ientry].XX,fblock_1,2);   
	char file_content[1024];
	int input_length=0;
	char ch;
	while ((ch = getchar()) != 27 && input_length < 1024) {
		file_content[input_length++] = ch;
	}
	if(input_length<1024){
		flag1=1;
		file_content[input_length] = '\0'; // Null terminate the string
	}
	writeSFS(fblock_1, file_content);
}
//inserting file data in second block
if(flag1==0){
	int fblock_2;
	if ((fblock_2 = getBlock())==-1) {  // first get a new block using the block bitmap
	printf("Error: Disk is full.\n");
	return;
	}else{
		writeSFS(fblock_2,NULL); // write all zeros to the block (there may be junk from the past!)
		itos(_inode_table[empty_ientry].YY,fblock_2,2);
		char file_content[1024];
		int input_length=0;
		char ch;
		while ((ch = getchar()) != 27 && input_length < 1024) {
		...
		writeSFS(fblock_2, file_content);
	}

	//inserting file data in third block
	if(flag2==0){
		int fblock_3;
		if ((fblock_3 = getBlock())==-1) {  // first get a new block using the block bitmap
		printf("Error: Disk is full.\n");
		return;
		}else{
			writeSFS(fblock_3,NULL); // write all zeros to the block (there may be junk from the past!)
			itos(_inode_table[empty_ientry].ZZ,fblock_3,2);
			char file_content[1024];
			int input_length=0;
			char ch;
			while ((ch = getchar()) != 27 && input_length < 1024) {
			...
			file_content[input_length] = '\0'; // Null terminate the string
			writeSFS(fblock_3, file_content);
		}
	}
...
/****************************************************************************/
/* delete the contents of the file or directiry <fname> within the current directory if
/* it exists
/*
/****************************************************************************/

void deletion(int e_inode){
	...
	itype = _inode_table[e_inode].TT[0];
	...
	for (i=0; i<3; i++) {
		...
		for (j=0; j<4; j++) {
			...
			int  del_inode = stoi(_directory_entries[j].MMM,3); // this is the inode that has more info about this entry			
			if (_inode_table[del_inode].TT[0]=='D'){
				deletion(del_inode);
				returnInode(del_inode);
			}else{
				int del_blocks[3];
			
				del_blocks[0] = stoi(_inode_table[del_inode].XX,2);
				del_blocks[1] = stoi(_inode_table[del_inode].YY,2);
				del_blocks[2] = stoi(_inode_table[del_inode].ZZ,2);
				for(int k=0;k<3;k++){
					if(del_blocks[i]!=0){
						//writeSFS(del_blocks[i],NULL);
						returnBlock(del_blocks[i]);
					}
				}
				returnInode(e_inode);
			}
...
void rm(char *fname) {
		...
		for ( j=0; j<4; j++) {
			...
			if (strncmp(fname,_directory_entries[j].fname, 252) == 0) { // and it is the one we are looking for
				...
				if (_inode_table[e_inode].TT[0]=='D'){
					...
					del_blocks[0] = stoi(_inode_table[e_inode].XX,2);
					del_blocks[1] = stoi(_inode_table[e_inode].YY,2);
					del_blocks[2] = stoi(_inode_table[e_inode].ZZ,2);
					_directory_entries[j].F='0';
					writeSFS(blocks[i],(char *)_directory_entries);
					for(int i=0;i<3;i++){
						if(del_blocks[i]!=0){
	...
	if (!found) printf("%.252s: No such file.\n",fname);
...
void display(char *fname) {
	...
	if (found) { // display file contents to standard output
		int block_file_data[3];
				block_file_data[0] = stoi(_inode_table[e_inode].XX,2);
		block_file_data[1] = stoi(_inode_table[e_inode].YY,2);
		block_file_data[2] = stoi(_inode_table[e_inode].ZZ,2);
		int i;
		for(i=0;i<3;i++){
			char output[1024];
			if(block_file_data[i]==0) continue;
			readSFS(block_file_data[i],output);
			printf("%s",output);
		}
